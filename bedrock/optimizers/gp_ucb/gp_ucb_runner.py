...
class GPUCBRunner(RunnerBase):
    ...
    def run(self):
        ...
        if self.params['dataset'] == 'MSSynthetic1D':
            ...
            params_setting['is_source_concat_for_naive'] = self.params['is_source_concat_for_naive']
            params_setting['is_separate_source_dens'] = self.params['is_separate_source_dens']

        json.dump(params_setting, open('{}/params.json'.format(self.params['path']), 'w'), indent=4, sort_keys=True)
