...
def get_args(rundeck=False):
    ...
    if not rundeck:
        ...
        args.obj_func = None
    args.path = output_dir(args.dataset, args.estimator_type, args.optimizer, args.tuning_path)
...
def add_args(subparser):
    ...
    c('--alpha', type=float, default=0.0, help='for calculating alpha-relative density ratio')
...
def read_info_from_json(args, params_json_path):
    with open(params_json_path) as f:
        ...
        if 'scatter_ylim' in json_params.keys():
            args.scatter_ylim = [json_params['scatter_ylim'][0], json_params['scatter_ylim'][1]]
        if 'flag_yscale_log' in json_params.keys():
            args.flag_yscale_log = distutils.util.strtobool(json_params['flag_yscale_log'])
